...
8. **Submission format.** RFC-0022's structured-schema addendum is superseded by this RFC, but the `ContractSubmission` format (the artifact a contractor hashes and commits as `submissionHash`) is not yet defined here. The submission is a content-addressed artifact like the exam itself, but its internal structure — how answers reference problem identities, how multi-stage submissions compose — depends on the chosen artifact format. This should be specified before implementation; a minimal sketch: `{ schema_version: "1", exam_id, stage, answers: [{ problemId, value }] }`, with `value` typed by problem kind.
