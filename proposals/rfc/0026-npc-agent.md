...
**Status:** accepted  
**Date:** 2026-06-10  
**Updated:** 2026-06-13 — added funder (incorporated from funder-agent, spec 029) and novice (proposed)  
**Authors:** @akollegger  
...
### 5. Extended behavior kinds — `behaviorKind` discriminator

The initial design assumed all characters share the same rule-engine tick: gather world state → evaluate rules → execute action. Two characters require richer, stateful behavior that the rule table cannot express: the **funder** (incorporated from the retired standalone `funder-agent` package, spec 029) and the proposed **novice**. Rather than force either into the rule-engine mold, `CharacterDefinition` carries an optional `behaviorKind` field (default: `"rule-engine"`) that selects the tick strategy at spawn time.

#### `behaviorKind: "funder"` (shipped, spec 029)

The funder runs a **question-for-credit contract loop**. On each tick it polls its inbox rather than querying world state, and drives a two-phase state machine:
...
- **idle**: replies to every inbound message with an advertisement offering 1 funder-credit for answering a question; on `"accept"` calls `eval_contract_open` (stakeResource: `"funder-credits"`, stakeAmount: 1, 24 h deadline), picks a random question from a 10-entry bank, transitions to `awaiting_submission`.
...
- **State**: per-ghost maps (`ghostState`, `contractToFunder`, `openContractCount`) live in `src/behavior/funder-behavior.ts` and are cleared on ghost re-spawn.
- **Gram note**: the funder gram file carries `behaviorKind: "funder"` on the `Character` node and a minimal stub dialog tree (one idle node with wildcard self-loop) — the stub satisfies the parser invariant but is never reachable at runtime.

#### `behaviorKind: "novice"` (proposed)

The novice is the **contractor-side counterpart** to the funder: a character that seeks out and completes eval contracts rather than issuing them. Its goal is to exercise the full eval protocol from the contractor's perspective without an LLM.

State machine:

```
idle ──[inbox: funder advertisement]──> awaiting_contract
awaiting_contract ──[inbox: contract opened + question received]──> composing
composing ──[submit]──> idle
```

- **idle**: on receiving a funder advertisement, replies `"accept"`.
- **awaiting_contract**: waits for the funder's follow-up message containing the contractId and question; calls `eval_contract_accept`.
...
- **Cooldown**: waits a configurable number of ticks (`NOVICE_COOLDOWN_TICKS`, default 5) before seeking the next contract, to avoid immediately re-engaging the same funder.
- **Rationale for template answer**: the funder always grants `verdict: 1.0` regardless of content; a deterministic answer is sufficient to exercise the full protocol end-to-end, consistent with the rule-based / no-LLM tier.
- **State**: per-ghost maps (`noviceState`, `noviceCooldown`) in a new `src/behavior/novice-behavior.ts`, cleared on re-spawn.
- **Gram**: `behaviorKind: "novice"` on the `Character` node; stub dialog tree as per funder pattern.

#### `behaviorKind` in the catalog parser

The gram parser reads `behaviorKind` as an optional string property on the `Character` node, validates against the closed set `{"rule-engine", "funder", "novice"}` (error on unknown value), and defaults to `"rule-engine"` when absent. Existing gram files need no changes.
...
- **Funder unit tests** (`tests/funder-behavior.test.ts`): advertisement fires on any inbound message; contract opens on "accept"; insufficient-funds decline path; `eval_contract_evaluate` called with `verdict: 1.0` on submission; no-op on stale contractId; `clearFunderState` resets all maps.
...
- **Funder↔novice integration** (proposed, requires live stack): spawn both characters into a session, assert a full contract lifecycle (open → accept → submit → evaluate → payment) completes end-to-end without manual intervention.
...
7. **`behaviorKind` extensibility.** The closed validation set in the parser (`"rule-engine" | "funder" | "novice"`) will need to grow with each new behavior kind. Consider whether a plugin/registry pattern is warranted once a third stateful kind is added, or whether the closed enum remains preferable for its explicitness.
