...
## Recent Changes
- 035-resilient-service-components: Resilient service components. `agent-host` now uses a Redis-backed catalog (`REDIS_URL` env var; falls back to file) so registrations survive pod restarts. A heartbeat endpoint (`POST /v1/catalog/:agentId/heartbeat`) lets agents report liveness. `agent-host` startup pings each `rosterAgent` and spawns ghosts eagerly if a session is active. `random-agent` sends periodic heartbeats and reconciles its ghost roster when the session ID changes. `npc-agent` wraps each ghost's MCP loop with `Effect.retry` + exponential backoff (2s→60s); emits `npc-agent.mcp.degraded`/`recovered` events per ghost per state transition. All three `/health` endpoints now reflect real degraded state. Set `REDIS_URL=redis://redis:6379` in deployment; set `AGENT_HOST_DISABLE_RECONCILIATION=1` to opt out of startup reconciliation.
