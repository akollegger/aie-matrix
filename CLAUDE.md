...
## Recent Changes
- 029-funder-into-npc: Migrated funder character into npc-agent as a `behaviorKind: "funder"` dispatch path. `funder-agent` container removed from `deploy/staging/docker-compose.yml`. New `funder-behavior.ts` module holds per-ghost state machine; `CharacterDefinition` extended with `behaviorKind` discriminator; `funder.character.gram` added to npc-agent catalog.
