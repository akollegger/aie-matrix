...
## Active Technologies
- **pnpm** workspaces for the TypeScript monorepo; TypeScript on Node.js **24+** for `server/*`, `client/phaser`, `shared/types`, and `ghosts/*` packages; Python 3.11+ for `ghosts/python-client/` stub only. Colyseus (authoritative room + WebSocket broadcast to spectators); Phaser 3 (spectator); MCP server/client libraries for `world-api` and SDKs; **Effect-ts** for combined-server orchestration (DI, typed errors, tracing — phased per `proposals/adr/0002-adopt-effect-ts.md`); Vite for Phaser dev; minimal HTTP stack for `registry` and static or dev-server delivery of Phaser build; JWT handling in `auth` (dev secret for PoC).
- **Storage (PoC):** in-memory Colyseus room state, ephemeral registry/adoption data, and human-authored map files under `maps/`; no durable DB for the minimal PoC unless a later task adds a small on-disk fixture.
...
- 002-effect-ts-transition: ADR-0002 accepted — Effect-ts for server orchestration; `specs/001-minimal-poc/plan.md` and agent context aligned with decided stack in `docs/architecture.md`.
