...
One terminal; **Ctrl+C** stops server, spectator, agent host, and random-agent (five in-world ghosts when using `--ghosts 5`). Orchestration lives in [`scripts/demo.mjs`](scripts/demo.mjs). For debugging, use separate **`pnpm run server`**, **`pnpm run spectator`**, and agent-host / random-agent shells instead.
