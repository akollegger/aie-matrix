...
/** Same fallback peppers used historically — shows when registry hasn't
 *  answered yet, or when the ghost has no registered displayName. */
function fallback(ghostId: string): string {
  return ghostId.length > 8 ? `ghost_${ghostId.slice(0, 8)}` : ghostId;
}
