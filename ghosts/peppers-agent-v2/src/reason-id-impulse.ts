...
export interface InvokeImpulseRequest {
  ...
  /** Felt-vocabulary fullness signal from `fullnessFelt(needs)`. Fires
   *  at Fuel ≥ setpoint + 0.5, before the primal drive does — gives
   *  the impulse an explicit "you do not need food right now" cue
   *  so the action-pull doesn't reach for `consume` out of habit. */
...
export async function invokeImpulse(
  ...
  if (req.primalDrive) {
    ...
    lines.push("");
  } else if (req.fullness) {
    // No drive firing, but the body still has something to say.
    // The fullness signal nudges the impulse away from `consume`
    // before drive territory.
    lines.push(`Body feels: ${req.fullness}.`);
    lines.push("");
  }

  lines.push("Slider profile (your INTERNAL felt state — shapes what kind of action pulls at you):");
