 ...
 *
 *   pnpm --filter @aie-matrix/ghost-peppers-agent-v2 run start
 */
...
// Sleep pipeline (Step E/F experiment wiring): PEPPERS_SLEEP_AT_CASCADE=N
// schedules a BLACKOUT at cascade N for the ghost at index
// PEPPERS_SLEEP_GHOST_INDEX (default 0 — the subject). Other ghosts in
// the same run never sleep — they are the controls.
const sleepAtEnv = process.env.PEPPERS_SLEEP_AT_CASCADE;
const sleepAtCascade =
  sleepAtEnv !== undefined && sleepAtEnv !== "" ? Number(sleepAtEnv) : undefined;
if (sleepAtCascade !== undefined && (!Number.isInteger(sleepAtCascade) || sleepAtCascade < 1)) {
  throw new Error(`PEPPERS_SLEEP_AT_CASCADE must be a positive integer; got ${sleepAtEnv}`);
}
const sleepGhostIndex = Number(process.env.PEPPERS_SLEEP_GHOST_INDEX ?? "0");
if (sleepAtCascade !== undefined) {
  console.info(
    `[peppers-house] sleep experiment: ghost #${sleepGhostIndex} blacks out at cascade ${sleepAtCascade}; all others are controls`,
  );
}
...
const runs = personalitiesAndSeeds.map(({ state }, i) =>
  runHouse({
    ...
    ...(sleepAtCascade !== undefined && i === sleepGhostIndex
      ? { sleepAtCascade }
      : {}),
  }),
);
