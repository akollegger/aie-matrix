...
/**
 * Session-wide recent conversation for one ghost — the last N messages it has
 * exchanged with ANYONE, oldest → newest. Unlike {@link fetchRecentDialogueWith}
 * (filtered to specific peers), this is the ghost's whole recent conversational
 * stream, so the Surface always has its short-term conversation memory even when
 * the current peer isn't in the immediate cluster snapshot. Reads only the
 * un-consolidated `:Message` window — sleep consolidation relabels older turns to
 * `:ConsolidatedMessage`, so this naturally bounds.
 *
 * Outgoing (role="assistant") turns are the ghost's own words; incoming
 * (role="user") turns are stored "<from>: <text>" so the speaker is already in
 * the content.
 */
export async function fetchRecentConversation(
  ...
  const result = await callOrThrow(client, "graph_query", {
    query: `
      MATCH (c:Conversation { session_id: $self_id })-[:HAS_MESSAGE]->(m:Message)
      WITH m ORDER BY m.timestamp DESC LIMIT $k
      WITH m ORDER BY m.timestamp ASC
      RETURN m.role AS role, m.content AS content, m.metadata AS metadata, toString(m.timestamp) AS at
    `,
    parameters: { self_id: ghostId, k: limit },
  });
  const turns: DialogueTurn[] = [];
  for (const r of rowsOf(result)) {
    const text = stringOrNull(r.content);
    if (text === null) continue;
    const by: "self" | "other" = stringOrNull(r.role) === "assistant" ? "self" : "other";
    turns.push({
      by,
      cascadeIndex: parseCascadeIndexFromMetadata(r.metadata),
      at: stringOrNull(r.at),
