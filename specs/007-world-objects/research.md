...
- Colyseus MapSchema: would give Phaser spectators real-time updates automatically, but mixes object placement state into the room schema, which is owned by `server/colyseus/`. Object state broadcast to Colyseus should be an explicit push from `ItemService` to the Colyseus bridge (same pattern as ghost mode in Decision 2 of 006-ghost-conversation).
...
**Decision**: On every `take` and `drop` mutation, `ItemService` calls a new `WorldBridgeService` method `setTileObjects(h3Index, itemRefs[])` that sets a Colyseus `MapSchema<string>` of `h3Index → comma-separated itemRef list`. A separate `MapSchema<string>` for `ghostId → comma-separated itemRef list` broadcasts ghost inventories.
...
## Decision 3: Map Loader Extension — `item-placement` Layer and Sidecar

**Decision**: Extend `loadHexMap()` in `server/colyseus/src/mapLoader.ts` to accept an optional `itemsPath?: string` parameter (the resolved sidecar path). The caller (`server/src/index.ts` via `ServerConfigService`) is responsible for resolving `AIE_MATRIX_ITEMS` → absolute path before passing it in. If `itemsPath` is not provided, `loadHexMap()` falls back to the co-located convention (`<map-dir>/<map-basename>.items.json`). The loader:
1. Reads the sidecar from the resolved path (or co-located fallback). Missing file → empty sidecar, no error.
2. Reads an optional layer named `"item-placement"` from `tmj.layers` (by name, not by index).
3. Returns both in an extended `LoadedMap` type: `itemSidecar: Map<string, ItemDefinition>` and `initialItemRefs` per `CellRecord`.

**Rationale**: Decoupling the sidecar from the map path enables mixing different combinations of map, rules, and objects — the same map can be loaded with different object sets by changing `AIE_MATRIX_ITEMS`, and the same object set can be shared across multiple maps. This follows the exact pattern of `AIE_MATRIX_MAP` + `AIE_MATRIX_RULES`: each resource type has its own env var and an independent default. The loader itself does not read `process.env` — path resolution stays in `ServerConfigService` and `parseServerConfigFromEnv()`, consistent with how `mapPath` is already handled.

**`AIE_MATRIX_ITEMS` env var**:
- Unset → fall back to co-located `<map-dir>/<map-basename>.items.json` (backward-compatible default)
...
- Load sidecar separately in `ItemService`: requires exposing the path outside the config layer; duplicates path resolution.
- Extend `mapLoader.ts` only for tileset `objects` properties but handle the `item-placement` layer in `ItemService`: splits map-parsing concerns across two packages.

---

## Decision 4: `ItemService` Package Placement

**Decision**: Add `ItemService` as an Effect `Context.Tag` service in `server/world-api/src/ItemService.ts`. It is not a new package.

**Rationale**: The feature adds `inspect`, `take`, `drop`, and `inventory` MCP tools — all in `server/world-api/`. The `ItemService` is logically a peer of `WorldBridgeService` and `MovementRulesService`. Creating a new `server/objects/` package would require a new `package.json`, build config, and inter-package import — unjustified for five new exported functions. No new top-level repository directories or packages are introduced.
...
**Alternatives considered**:
- Capacity check inside `ItemService`: would require `ItemService` to know about ghost positions, creating a circular dependency with `WorldBridgeService`.

---

## Decision 6: `itemRef` Identity in MCP Tools

**Decision**: All MCP tools (`inspect`, `take`, `drop`) use `itemRef` (the sidecar key) as the identifier in their `inputSchema`. When a tile has multiple objects with the same `itemRef` (e.g., three chairs), `take` selects the first available relationship and does not expose instance selection to the ghost agent.
...
**Decision**: Extend `CellRecord` to add an optional `capacity?: number` (currently absent from the type — it's read by Neo4j seed code directly from the tileset). Add a new `itemSidecar` field to `LoadedMap`. Add `initialItemRefs` to `CellRecord` to carry the per-cell declared objects from map load time. `ItemService` consumes `initialItemRefs` at construction to build its in-memory state.
