...
3. Start combined server from the repo root (`pnpm run server` or `pnpm run server:dev`; see [quickstart.md](../quickstart.md)).
4. Start Phaser dev client (`pnpm run spectator`) and open the printed URL; confirm the map renders (ghosts optional).
5. Run **developer adoption flow** — `pnpm run ghost:house` and/or the `curl` sequence in [`server/registry/README.md`](../../../server/registry/README.md) targeting **`ghosts/random-house/`** so a house registers, adopts, and the walker runs.
...
Prefer one process with `pnpm --filter @aie-matrix/ghost-random-house start -- --ghosts 2` (one house, two caretakers), or two terminals each running `pnpm run ghost:house` (two houses). IC-002 requires a distinct caretaker per ghost.
