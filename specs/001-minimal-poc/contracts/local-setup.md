...
1. Install dependencies (`npm install` or workspace equivalent at repo root when scaffold exists).
2. Build packages if required by toolchain.
3. Start combined server from `server/` (`npm run dev` per RFC — exact script name to match implementation).
...
7. Run `npm test` (or documented command) from **`/Users/akollegger/Developer/akollegger/aie-matrix/ghosts/tck/`** — all TCK steps pass.
