...
## Provider registration (AgentHost)

- ** Preconditions**: Valid operator/auth as required by PoC dev middleware.
- **Behavior**: Register a AgentHost provider; receive `agentHostId` and any server-assigned metadata needed for subsequent adoption calls.
...
- **Input**: `caretakerId`, `agentHostId`, ghost selection parameters as needed (PoC may fix a single ghost template from `random-house`).
