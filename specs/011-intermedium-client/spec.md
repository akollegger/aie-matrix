...
- **FR-026** (Exterior stops): At exterior stops (Global, Regional, Neighborhood) the board MUST be rendered as an extruded `H3HexagonLayer`. Ghost positions MUST NOT be shown. The camera MUST point at the board’s geographic center for all three exterior stops.
- **FR-027** (Camera pitch): Each stop MUST use a defined camera pitch: 0° for Global, Regional, Plan, and Room; 45° for Neighborhood and Situational; approximately 80° for Personal (R3F scene). Pitch MUST animate as part of each stop-to-stop transition.
