...
- **FR-006**: The client MUST fetch world map topology (tile types, item placements, H3 cell graph) from the HTTP map endpoint at startup and parse the `.map.gram` layered format via `@aie-matrix/map-gram`; regex-based parsing of the old flat-cell format is superseded by `specs/013-gram-format-migration`.
