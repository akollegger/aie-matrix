# Feature Specification: Agent Host A2A Coordination

**Feature Branch**: `009-agent-host-a2a`  
**Created**: 2026-04-24  
**Status**: Draft  
**Input**: Ghost agents collaborating and coordinating through the Agent Host over A2A

## Proposal Context *(mandatory)*

- **Related Proposal**: [ADR-0004: A2A as the Ghost Agent Protocol](../../proposals/adr/0004-a2a-ghost-agent-protocol.md) · [RFC-0007: Agent Host Architecture](../../proposals/rfc/0007-agent-host-architecture.md)
- **Scope Boundary**: Build the canonical agent host service — A2A host, MCP proxy, Colyseus bridge, agent supervisor, and catalog — so that ghost agents can be registered, spawned, supervised, and fed world events. Covers all three behavioral tiers (Wanderer, Listener, Social) across three delivery phases. ADR-0004 and RFC-0007 are the authoritative design documents; implementation must stay synced with them, and significant deviations require explicit approval before merging.
...
A third-party contributor implements a Wanderer-tier ghost agent (MCP only: random or rule-based movement, no social behavior), registers it with the agent host catalog, and sees it running as a live ghost in the Matrix world.
...
**Independent Test**: Register the reference `random-agent` with the agent host catalog, assign it to a ghost, and confirm the ghost moves autonomously in the world. Delivers a live ghost without any other tier being built.

**Acceptance Scenarios**:

1. **Given** a agent host running with an empty catalog, **When** a contributor POSTs a valid agent card (with `matrix` extension, `tier: "wanderer"`) to the catalog registration endpoint, **Then** the agent card is stored and retrievable from the catalog.
2. **Given** a registered Wanderer agent, **When** the agent host spawns it for an adopted ghost, **Then** the agent receives its spawn context (ghost id, world entry point) and begins emitting `go` actions via MCP.
...
A Listener-tier ghost agent is registered with the agent host and receives translated world events — proximity changes, incoming messages, quest triggers — pushed to it from the Colyseus bridge via A2A.
...
1. **Given** a Listener agent registered in the agent host catalog, **When** another participant in the world sends a message to the ghost, **Then** the Colyseus bridge translates the event and the A2A host pushes it to the agent within observable latency.
...
**Why this priority**: Enables the signature ghost behavior: two-way conversation between ghosts and with human attendees. This is the full social loop and requires the complete agent host pipeline.
...
### User Story 4 — Core team operates the agent host reliably (Priority: P2)

The core team can deploy, configure, and observe the agent host as a single service that manages all spawned agents, enforces tier conformance, and survives agent failures without manual intervention.

**Why this priority**: Operational reliability is required before any contributed agents run at the event. The agent host is a single point of failure and must operate autonomously for the duration of AIEWF 2026.

**Independent Test**: Start the agent host with two agents registered (one Wanderer, one Social). Kill one agent process. Confirm the supervisor detects the failure, attempts restarts per the retry policy, and logs the event. The other agent continues unaffected.

**Acceptance Scenarios**:

1. **Given** the agent host is running, **When** a health check ping to a spawned agent times out (30s), **Then** the supervisor marks the agent unhealthy, begins restart with exponential backoff, and logs the failure.
...
3. **Given** a ghost that is released (adoption ends), **When** the agent host initiates shutdown for its agent, **Then** the agent receives a graceful cancellation and is hard-killed after 10 seconds if it does not comply.
4. **Given** the agent host catalog, **When** a new agent card is published with a `matrix.tier` that does not match the declared A2A capabilities, **Then** the catalog rejects the registration with a descriptive error.
...
- How does the agent host handle a Wanderer agent that accidentally sends A2A `say` actions it is not supposed to emit?
...
- **FR-001**: The agent host MUST expose a catalog registration endpoint that accepts A2A agent cards with a valid `matrix` extension object.
...
- **FR-003**: The catalog MUST store registered agent cards in a file-backed persistent store so that contributed agents survive agent host restarts.
...
- **Agent Host**: The single canonical service that hosts ghost agents; exposes A2A host, MCP proxy, Colyseus bridge, agent supervisor, and catalog as internal components.
...
- **IC-004**: World event envelope — the A2A event payload format for translated Colyseus events (`message.new`, proximity, quest, session). Format validated by spike-008 (IC-008 envelope); must be canonicalized in the agent host implementation guide.
...
- **SC-002**: The agent host delivers world events to Listener and Social agents with end-to-end latency low enough that ghost reactions feel responsive to human observers in the world.
...
- **SC-007**: The agent host operates continuously across the AIEWF 2026 event duration without requiring manual restarts of the host service itself.
...
- The world server (Colyseus) and its MCP tools are available and stable before agent host development begins; the agent host builds on top of the existing MCP wire protocol from ADR-0001.
...
- Colyseus schema changes that break the agent host bridge require coordination with the world server team; the bridge is an internal implementation detail not exposed to contributed agents.
...
- **RFC-0007** (`proposals/rfc/0007-agent-host-architecture.md`) — MUST be updated when component boundaries, interfaces, HTTP paths, capability manifest, or the agent card schema are changed from the RFC draft. Open questions in §Open Questions MUST be resolved and struck or answered as implementation proceeds.
...
- `CLAUDE.md` — MUST be updated with the agent host package and its technology stack under Recent Changes when the package ships.
