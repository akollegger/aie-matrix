...
| Command | When to use |
|---------|-------------|
| `pnpm run demo` | One terminal: combined server + Vite spectator + **@aie-matrix/server-agent-host** + **@aie-matrix/random-agent** (Wanderer on localhost; see comment block in `scripts/demo.mjs`) |
| `pnpm run server` / `pnpm run server:dev` | Server only (production-like `start` vs `tsx watch`) |
| `pnpm run spectator` / `pnpm run ghost:house` | Phaser spectator, or **agent host** alone (`@aie-matrix/server-agent-host`) for multi-shell debugging |
...
### Ghost agents (A2A agent host)

Third-party **ghost agents** are hosted by the **agent host** package (`@aie-matrix/server-agent-host`) and speak **A2A** to the house, not directly to the browser. The normative feature spec and contracts live under [`specs/009-agent-host-a2a/`](specs/009-agent-host-a2a/).
