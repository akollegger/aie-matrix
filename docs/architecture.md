...
| **Agent-host ↔ First-party ghosts** | HTTP A2A: agent card at `/.well-known/agent-card.json`, `message/send` for tasks and world events, catalog and session control on the agent-host HTTP API (IC-005). Ghosts self-register at startup via `POST /v1/catalog/register { agentId, baseUrl }` and deregister on SIGTERM. In compose and K8s, `baseUrl` is the container's service-DNS or pod-IP URL. **Agent-callable self-spawn (ADR-0012):** a registered agent may call `POST /v1/sessions/spawn-roster/:agentId` (authenticated by its MCP session token) to spawn a roster of characters in one batch; per-character failures do not abort the batch. The world server emits `world.session.start` when a live session becomes active — agents that declare `"world.session.start"` in their `worldEventSubscriptions` card field receive this event and can trigger roster spawn without needing an active session check at startup. |

**Contracts:** `specs/009-agent-host-a2a/contracts/` — in particular **IC-001** (agent card `matrix` block), **IC-002** (A2A + push invariants), **IC-003** (MCP tool surface), **IC-004** (world event envelope), **IC-005** (catalog HTTP), **IC-006** (spawn context). See also **IC-006** in `specs/028-npc-agent/contracts/` for the agent-callable spawn-roster endpoint and **IC-007** for the `world.session.start` emission contract.
...
| **Session leaderboards** | `server/world-api/` — `LeaderboardService` / `LeaderboardServiceLive` | Spectator-facing read model computed from ledger entries. A map's `[leaderboards:Leaderboards | ...]` block declares which aggregations are in play. Rankings are recomputed on demand; the `game-end` calendar event triggers `finalize-leaderboards`, which freezes the final snapshot. Emits `world.leaderboard.updated` A2A event on each snapshot. `(:LeaderboardSnapshot)` nodes in Neo4j; in-memory TTL cache for live rankings. Specified in [RFC-0025](../proposals/rfc/0025-session-leaderboards.md), implemented in [spec 026](../specs/026-session-leaderboards/). |
...
| **Agent-host + first-party ghosts** | `server/agent-host/`, `ghosts/random-agent/`, `ghosts/npc-agent/` | Canonical house: catalog, MCP proxy, Colyseus bridge, A2A supervisor. `random-agent` is the reference Wanderer: containerized per ADR-0009, self-registers on startup, deregisters on SIGTERM. `npc-agent` is the reference NPC roster: reads `.character.gram` catalog files, spawns named ghosts per session via agent-callable self-spawn (ADR-0012), drives each via behavior rules and scripted dialog. Add new ghosts following `ghosts/README.md`. |
